...
export class Migration20260410173217 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table "processing_items" ("id" uuid not null default gen_random_uuid(), "job_id" uuid not null, "organization_id" uuid not null, "tenant_id" uuid not null, "item_index" int not null, "status" text not null default 'pending', "current_step" text null, "input_data" jsonb not null, "output_data" jsonb null, "step_results" jsonb null, "selected_values" jsonb null, "error_message" text null, "created_at" timestamptz not null, "updated_at" timestamptz not null, constraint "processing_items_pkey" primary key ("id"));`);
    this.addSql(`create index "processing_items_job_id_status_index" on "processing_items" ("job_id", "status");`);
    this.addSql(`create index "processing_items_job_id_index" on "processing_items" ("job_id");`);

    this.addSql(`create table "processing_jobs" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "pipeline_id" uuid not null, "name" text null, "status" text not null default 'pending', "current_step" text null, "total_items" int not null default 0, "processed_items" int not null default 0, "failed_items" int not null default 0, "skipped_items" int not null default 0, "progress_job_id" uuid null, "source_type" text null, "source_id" uuid null, "config" jsonb null, "result_summary" jsonb null, "started_at" timestamptz null, "completed_at" timestamptz null, "created_by" uuid null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "processing_jobs_pkey" primary key ("id"));`);
    this.addSql(`create index "processing_jobs_pipeline_id_index" on "processing_jobs" ("pipeline_id");`);
    this.addSql(`create index "processing_jobs_status_index" on "processing_jobs" ("status");`);
    this.addSql(`create index "processing_jobs_tenant_id_organization_id_index" on "processing_jobs" ("tenant_id", "organization_id");`);

    this.addSql(`create table "processing_pipelines" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "pipeline_key" text not null, "name" text not null, "description" text null, "steps" jsonb not null, "webhook_url" text null, "is_active" boolean not null default true, "created_by" uuid null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "processing_pipelines_pkey" primary key ("id"));`);
    this.addSql(`create index "processing_pipelines_tenant_id_pipeline_key_index" on "processing_pipelines" ("tenant_id", "pipeline_key");`);
    this.addSql(`create index "processing_pipelines_tenant_id_organization_id_index" on "processing_pipelines" ("tenant_id", "organization_id");`);
  }
